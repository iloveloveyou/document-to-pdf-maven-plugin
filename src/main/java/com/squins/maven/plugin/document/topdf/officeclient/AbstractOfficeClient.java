...
public abstract class AbstractOfficeClient implements OfficeClient {
	...
	private void createLibreOfficeExecutableWrapper() {
		...
		try {
			File file = createTempFile(
					"document-to-pdf-maven-plugin-libre-office-executable-wrapper",
					libreOfficeExecutableWrapperFilenameSuffix());
			writer = new FileWriter(file);
			writer.write(String.format("%s %s || exit 1",
					libreOfficeExecutable(), passScriptArguments()));
	...
	void handleProcessOutput(Process process) throws IOException {
		log.debug("Libre Office process stdout: "
	...
	@Override
	public Process startAndWaitLibreProcess(String... arguments)
			throws Exception {
		...
		if (process.exitValue() != 0) {
			throw new IllegalStateException("Process returned error exitValue:"
					+ process.exitValue());
		}
	...
	private ProcessBuilder buildLibreOfficeCommand(String... arguments) {
		...
		List<String> commandWithArgs = new ArrayList<String>(Arrays.asList(
		...
		log.info("Creating command: "
				+ StringUtils.join(commandWithArgs.iterator(), " "));
	...
	private void createLibreOfficeTemporaryUserProfileDirectory() {
		String path = System.getProperty("java.io.tmpdir") + "/loffice_profile"
				+ System.nanoTime();
